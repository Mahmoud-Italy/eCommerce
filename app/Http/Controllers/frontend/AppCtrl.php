  ...
  public function postPaymentWithStripe(Request $request)
 {
     ...
     if ($validator->passes()) { 
         $input = array_except($input,array('_token'));
          $stripe = Stripe::make('sk_test_oJN9Z5vR8pU3DAu4j6LDmKoD');
    ...
    if (!isset($token['id'])) {
      \Session::flash('error','Payment Transaction has error');
    ...
    $charge = $stripe->charges()->create([
         ...
         'currency' => 'USD',
         'amount' => 150,
         'description' => 'from e-commerce',
     ]);
 

    if($charge['status'] == 'succeeded') {
      \Session::flash('success','Payment Transaction Success');
   } else {
        \Session::put('error','Payment Not Correct');
   }
